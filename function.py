...
def save_marketing_lead_info(event, context):
    ...
    with leads_table.batch_writer() as batch:
        batch.put_item(Item=lead_object_from_params(
            request_params["marketing_lead"]))
...
def send_mail(context):
    SENDER = os.environ.get(
        'SENDER_SUPPORT_EMAIL') or "KL Marketing Enquiries <enquiries@kodelounge.com>"
...
def lead_object_from_params(lead_params):
    ...
    lead_params = {param: lead_params[param]
                   for param in permitted_keys if lead_params[param] is not ''}
...
def email_content(params):
  ...
  '''
    ...
    if 'subject' in params and params['subject'] is not '':
        str += 'Subject: {subject}<br>'

    if 'message' in params and params['message'] is not '':
        str += 'Message: <br> {message}'
